...
const SUGGESTIONS = [
  'Quelle offre internet proposez-vous ?',
  'Comment souscrire a la fibre ?',
  'Quels sont vos services pour entreprises ?',
];
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 0,
      role: 'bot',
      text: 'Bonjour ! Je suis l\u2019assistant OnePortal AI de CAMTEL. Posez-moi une question sur nos produits et services.',
    },
  ]);
    ...
    try {
      ...
      setMessages((m) => [
        ...m,
        {
          id: Date.now() + 1,
          role: 'bot',
          text: "Je n'ai pas pu joindre le service d'assistance pour le moment. Vous pouvez poser votre question via le formulaire de contact.",
        },
      ]);
    } finally {
      setIsTyping(false);
          ...
          >
            <div className="flex items-center justify-between bg-gradient-to-r from-primary-900 via-primary to-primary-600 px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-white">OnePortal AI</p>
                  <p className="flex items-center gap-1 text-[11px] text-white/80">
                    <span className="inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" /> Assistant CAMTEL
                  </p>
                </div>
              </div>
              <button
