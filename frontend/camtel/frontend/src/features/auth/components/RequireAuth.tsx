...
export function RequireAuth({ roles, permission }: { roles?: UserRole[]; permission?: Permission }) {
  ...
  if (isLoading) {
    return <div className="flex h-screen items-center justify-center text-neutral-400">Chargement...</div>;
  }
  ...
  if (isForbidden) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-2 text-center">
        <p className="text-lg font-semibold text-neutral-800">Acces refuse</p>
        <p className="text-sm text-neutral-500">Votre role ne permet pas d'acceder a cette page.</p>
      </div>
    );
  }
